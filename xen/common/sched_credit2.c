...
static void parse_credit2_runqueue(const char *s)
{
    ...
    printk(XENLOG_WARNING "Unrecognized value of credit2_runqueue option!\n");
}
    ...
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
    ...
    if ( !is_idle_vcpu(scurr->vcpu) && scurr->rqd != rqd)
    {
        ...
        printk(XENLOG_DEBUG "%s: pcpu %d rq %d, but scurr %pv assigned to "
               "pcpu %d rq %d!\n",
               __func__,
               cpu, this_rqi,
...
csched2_init(struct scheduler *ops)
{
    ...
    printk(XENLOG_INFO " load_window_shift: %d\n"
    ...
    {
        printk(XENLOG_WARNING "%s: opt_load_window_shift %d below min %d, resetting\n",
